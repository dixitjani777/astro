...
setDocument = Sizzle.setDocument = function( node ) {
	...
	if ( preferredDoc != document &&
		...
		// Support: IE 9 - 10 only
			...
			subWindow.attachEvent( "unload", unloadHandler );
		}
	}
			...
			jQuery(_this).not('.js-advancified').addClass('js-advancified').on('change',function(e) {
				...
				*/
				...
				/*
					...
					is_multiple !== false
					Is used when "required" attribute is needed, for a single file
...
;(function ($) {
	...
	$.SOW.core.dropdown_ajax = {
		...
		ddImageSwitcher: function() {
			...
			// click
			// document.querySelectorAll(".dropdown-image-list").forEach((el) => {
			$.SOW.vanilla.each(".dropdown-image-list", function (el) {
				...
				$.SOW.vanilla.on(el, 'click', function(e) {
					...
					var _imageContainer 		= _t.querySelector('.dropdown-image-container') 				|| '',
						...
						/* loading classes */
						clsContainer 			= 'fi fi-orbit fi-spin fs--30 text-muted h--80 d-middle',
					...
					if(useCache+'' == 'true' && _cacheKey == window.ddimgCache) {
						...
						$.SOW.vanilla.ajax({
							...
							beforeSend: function() { 
								...
								// loading : add
								$.SOW.vanilla.addClass(ddAjaxContainer, clsContainer);
							...
							success: 	function(data) { 
								...
								// loading : remove
								$.SOW.vanilla.removeClass(ddAjaxContainer, clsContainer);
								_tpl = ''; // reset
					...
					function _updateBind() {
						...
						// ddContainer.querySelectorAll(".dropdown-image-item").forEach((item) => {
						var objUpdate = ddContainer.querySelectorAll(".dropdown-image-item");
						$.SOW.vanilla.each(objUpdate, function (item) {

							// item.addEventListener('click', (i) => {
							$.SOW.vanilla.on(item, 'click', function(i) {
								i.preventDefault();
					...
					// ddContainer.querySelectorAll(".dropdown-image-reset").forEach((item) => {
					var objReset = ddContainer.querySelectorAll(".dropdown-image-reset");
					$.SOW.vanilla.each(objReset, function (item) {
					...
					function __itemUpdate(img_src, img_id) {
						...
						$.SOW.vanilla.ajax({
							...
							beforeSend: function() { 
								...
								// loading : add
								$.SOW.vanilla.addClass(_imageContainer, clsItem);
							...
							success: 	function(data) { 
								...
								// loading : remove
								$.SOW.vanilla.removeClass(_imageContainer, clsItem);
...
;(function ($) {
	...
	$.SOW.core.utils = {
		...
		UtilLiveReload: function(selector) {
			...
			$.SOW.vanilla.each(selector, function (_t) {
				...
				/* ajax function call */
				$.SOW.vanilla.on(_t, _action, function(e) {
